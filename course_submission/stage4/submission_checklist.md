# Stage 4 Submission Checklist

- [ ] `course_submission/stage4/experiments_stage4.md` exists and contains all five experiments.
- [ ] `course_submission/stage4/final_demo.py` exists and runs locally.
- [ ] `course_submission/stage4/knowledge_base/mikizol_by_category.md` exists.
- [ ] Stage 3 Google Docs link is included in `stage3/README_stage3_document.md`.
- [ ] Google Docs access is set to "anyone with the link can view".
- [ ] If required by the course platform, Stage 3 document is exported/copied as `.docx` or `.pdf` into the final submission folder.
- [ ] Screenshot/video guidance exists in `screenshots/README_screenshots.md`.
- [ ] Final Drive/Yandex/Google folder link is ready for submission.
- [ ] No API keys, tokens, Authorization headers, private env dumps, server logs, or real customer data are included.
- [ ] Demo can run without Asterisk, Gateway, SSH, telephony infrastructure, or server credentials.
- [ ] Demo can run without OpenAI/API keys.
- [ ] Optional real GPT demo, if used later by the coordinator, is documented separately and is not required for the basic package.
