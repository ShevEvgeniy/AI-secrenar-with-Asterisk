...
- Current NODE-032 recommendation: `NODE-032BI / controlled-disabled-business-dialog-policy-field-live-smoke-after-helper-refresh`
- Rationale: NODE-032BH refreshed the deployed Asterisk helper/runtime reporting path and proved the NODE-032BF `business_dialog_transcript_*` fields appear in no-network safe diagnostics. The next separate approval-gated node should run one controlled disabled-path live smoke to prove those fields appear in the live smoke report.
