...
| `NODE-005-latency-and-turn-based-hardening` | `docs/nodes/NODE-005-latency-and-turn-based-hardening.md` | `feat/node-005-latency-and-turn-based-hardening` | Ready for live smoke | Reduce perceived latency inside the current turn-based architecture with stage-specific record contours, PHONE-only confirmation, and explicit latency tracing. | Local regression suite passed; transfer now requires `PHONE -> PHONE_CONFIRM -> positive confirmation -> transfer(from-internal,sales_real,1)`. |
